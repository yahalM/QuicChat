...
class WebSocketCommClient(CommClientInterface):
    ...
    async def open_connection(self):
        ...
        self._logger.info(f"Opening WebSocket connection to {self._config.uri}")
        self._websocket = await websockets.connect(self._config.uri)

    async def close_connection(self):
        # Concrete implementation to close the WebSocket connection
        self._logger.info(f"Closing WebSocket connection to {self._config.uri}")
        await self._websocket.close()

    async def send_message(self, message):
        # Concrete implementation to send a message via WebSocket
        self._logger.info(f"Sending message to {self._config.uri} via WebSocket: {message}")
        await self._websocket.send(message)

    async def receive_message(self):
        # Concrete implementation to receive a message via WebSocket
        received_message = await self._websocket.recv()
        self._logger.info(f"Received message from {self._config.uri} via WebSocket: {received_message}")
